...
def RSV(nine_day_data):
    ninth_day_close_value = float(
        nine_day_data[-1][CLOSE_INDEX].replace(",", ""))
    nine_day_min_low_value = min(
        [float(m[LOW_INDEX].replace(",", "")) for m in nine_day_data])
    nine_day_max_high_value = max(
        [float(m[HIGH_INDEX].replace(",", "")) for m in nine_day_data])
